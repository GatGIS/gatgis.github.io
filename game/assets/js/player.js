...
const playerLoadStats = () => {
    ...
    if (player.inCombat || playerDead) {
        ...
        playerInfoElement.innerHTML = `${player.name} Sem.${player.lvl} (${player.exp.expPercent}%)`;
    }

    // Header
    document.querySelector("#player-name").innerHTML = `<i class="fas fa-user"></i>${player.name} Sem.${player.lvl}`;
...
const lvlupPopup = () => {
    sfxLvlUp.play();
    addCombatLog(`Tu sasniedzi jaunu semestri! (Sem.${player.lvl - player.exp.lvlGained} > Sem.${player.lvl})`);
...
const generateLvlStats = (rerolls, percentages) => {
    ...
    const loadLvlHeader = () => {
        lvlupSelect.innerHTML = `
            ...
            <div class="content-head">
                <h4>Atlikuši: ${player.exp.lvlGained}</h4>
                <button id="lvlReroll">Pārlozēt ${rerolls}/2</button>
            </div>
        `;
    }
    loadLvlHeader();
    ...
    try {
        for (let i = 0; i < 4; i++) {
            ...
            p.innerHTML = `Palielini ${selectedStats[i].replace(/([A-Z])/g, ".$1").replace(/crit/g, "c").toUpperCase()} by ${percentages[selectedStats[i]]}%.`;
