window.addEventListener("load", function () {
    ...
    document.querySelector("#name-submit").addEventListener("submit", function (e) {
        ...
        if (format.test(playerName)) {
            document.querySelector("#alert").innerHTML = "Vārds nevar sastāvēt no īpašajām zīmēm!";
        } else {
            if (playerName.length < 3 || playerName.length > 15) {
                document.querySelector("#alert").innerHTML = "Vārdam jāsastāv no 3-15 zīmēm";
            } else {
                player = {
                    name: playerName,
                    lvl: 1,
    ...
    document.querySelector("#unequip-all").addEventListener("click", function () {
        ...
        <div class="content">
            <p>Novilkt visu ekipējumu?</p>
            <div class="button-container">
                <button id="unequip-confirm">Novilkt</button>
                <button id="unequip-cancel">Atcelt</button>
            </div>
        </div>`;
    ...
    document.querySelector("#menu-btn").addEventListener("click", function () {
        ...
        <div class="content">
            ...
            <button id="stats">Esošais mēģinājums</button>
            <button id="volume-btn">Skaņas iestatījumi</button>
            <button id="export-import">Export/Import Data</button>
            <button id="quit-run">Izstāties</button>
        </div>`;
        ...
        quitRun.onclick = function () {
            ...
            defaultModalElement.innerHTML = `
            <div class="content">
                <p>Vai tu vēlies izstāties?</p>
                <div class="button-container">
                    <button id="quit-run">Izstāties</button>
                    <button id="cancel-quit">Atcelt</button>
                </div>
            </div>`;
        ...
        volumeSettings.onclick = function () {
            ...
            <div class="content" id="volume-tab">
                ...
                <label id="master-label" for="master-volume">Kopējais (${master}%)</label>
                <input type="range" id="master-volume" min="0" max="100" value="${master}">
                <label id="bgm-label" for="bgm-volume">Fona mūzika (${bgm}%)</label>
                <input type="range" id="bgm-volume" min="0" max="100" value="${bgm}">
                <label id="sfx-label" for="sfx-volume">Skaņas efekti (${sfx}%)</label>
                <input type="range" id="sfx-volume" min="0" max="100" value="${sfx}">
                <button id="apply-volume">Apstiprināt</button>
            </div>`;
...
const allocationPopup = () => {
    ...
    const loadContent = function () {
        ...
        <div class="content" id="allocate-stats">
            ...
            <div class="row">
                <p id="alloPts">Statu Punkti: ${points}</p>
                <button id="allocate-reset">Reset</button>
            </div>
            <div class="row">
                <p>Passive</p>
                <select id="select-skill">
                    <option value="Remnant Razor">Paukmaģistra palīgs</option>
                    <option value="Titan's Will">Mūžīgais fuksis</option>
                    <option value="Devastator">Sausā ruka</option>
                    <option value="Blade Dance">Krustots ir</option>
                    <option value="Paladin's Heart">Oldermaņa sirds</option>
                    <option value="Aegis Thorns">Cirķeļa adata</option>
                </select>
            </div>
            <div class="row primary-panel pad">
                <p id="skill-desc">Uzbrukumi nodara 8% uzdevuma atlikušās dzīvības uz sitiena.</p>
            </div>
            <button id="allocate-confirm">Apstiprināt</button>
        </div>`;
    }
    defaultModalElement.style.display = "flex";
    ...
    const handleStatButtons = (e) => {
        ...
        if (e.includes("Add")) {
            ...
            if (points > 0) {
                ...
                document.querySelector(`#alloPts`).innerHTML = `Statu Punkti: ${points}`;
            } else {
                sfxDeny.play();
            }
