...
const hpValidation = () => {
    ...
    if (player.stats.hp < 1) {
        ...
        addCombatLog(`${enemy.name} tika uzveikts! (${new Date(combatSeconds * 1000).toISOString().substring(14, 19)})`);
        addCombatLog(`Tu ieguvi ${nFormatter(enemy.rewards.exp)} pieredzi.`)
        playerExpGain();
        addCombatLog(`${enemy.name} melnajā kasē ielika <i class="fas fa-coins" style="color: #FFD700;"></i>${nFormatter(enemy.rewards.gold)} latus.`)
...
const playerAttack = () => {
    ...
    addCombatLog(`${player.name} nogurdināja ${enemy.name} par ` + nFormatter(damage) + ` ${dmgtype}.`);
    hpValidation();
    playerLoadStats();
    enemyLoadStats();
...
const enemyAttack = () => {
    ...
    addCombatLog(`${enemy.name} nogurdināja ${player.name} par ` + nFormatter(damage) + ` ${dmgtype}.`);
    hpValidation();
    playerLoadStats();
    enemyLoadStats();
...
const updateCombatLog = () => {
    ...
    if (playerDead) {
        let button = document.createElement("div");
        button.className = "decision-panel";
        button.innerHTML = `<button id="battleButton">Uz Izvēlni</button>`;
        combatLogBox.appendChild(button);
    }
